...
export default function FormPeopleSummaryPanel({ payload, variant = 'client' }) {
  ...
  if (entries.length === 0) {
    if (isSolicitor) {
      return (
        <div className="flex items-start gap-3 rounded-xl border border-dashed border-slate-300 bg-slate-50/80 px-3 py-2.5 text-sm text-slate-600 sm:col-span-2">
  ...
  const stripClass = isSolicitor
    ? 'mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm sm:px-4 sm:col-span-2'
    : 'mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-600 bg-slate-800/95 px-3 py-2.5 shadow-inner sm:px-4';

  const textPrimary = isSolicitor ? 'text-slate-900' : 'text-slate-100';
  const textMuted = isSolicitor ? 'text-slate-500' : 'text-slate-400';
  const btnClass = isSolicitor
    ? 'inline-flex shrink-0 items-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-sm font-semibold text-indigo-900 hover:bg-indigo-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500'
