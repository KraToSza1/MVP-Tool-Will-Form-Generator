 ...
 * GuardianFlow.jsx
 * ----------------
 * Matches the existing website style exactly:
 * - Light background, white cards, indigo/purple accents
 * - Same radio button style, typography, and layout as the site
 *
 * Usage:
    ...
    <div style={S.section} className="min-w-0">
      ...
      {option === 'yes_same' && (
        <div style={S.blueCard}>
          <div style={S.blueCardTitle}>Who would you like to be your children&apos;s guardian?</div>
          <div style={S.blueCardHelp}>
      ...
      {option === 'yes_different' && !childrenConfirmed && (
        <div style={S.blueCard}>
          <div style={S.blueCardTitle}>Tell us about your children under 18</div>
          <div style={S.blueCardHelp}>
            Please add each child below. You will then be able to assign a specific guardian to each one.
          </div>

          {children.length === 0 && <div style={S.emptyState}>No children have been added yet.</div>}

          {children.length > 0 && (
            <>
              <div style={S.addedLabel}>✅ Added ({children.length}):</div>
              {children.map((c, i) => (
                <div key={i} style={S.itemRow}>
                  <span className="min-w-0 break-words">
                  ...
                  <div style={S.itemActions}>
                    <button type="button" style={S.editBtn} onClick={() => openEditChild(i)}>
                      ✏️ Edit
                    </button>
                    <button type="button" style={S.removeBtn} onClick={() => handleRemoveChild(i)}>
                      🗑 Remove
                    </button>
                  </div>
                </div>
          ...
          <button type="button" style={S.addBtn} onClick={openAddChild}>
            + Add Child
          </button>
          ...
          {children.length > 0 && (
            <>
              ...
              <button type="button" style={S.continueBtn} onClick={() => setChildrenConfirmed(true)}>
      ...
      {option === 'yes_different' && childrenConfirmed && (
        <div className="min-w-0">
          <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 4, color: C.textDark }}>
            Now assign a guardian to each child
          </div>
          <div style={{ fontSize: 14, color: C.textLight, fontStyle: 'italic', marginBottom: 20 }}>
            For each child below, add the person(s) you would like to be their guardian if both parents passed away.
          </div>

          {children.map((child, i) => (
            <div key={i} style={S.childBlock}>
              <div style={S.childName}>
                {child.childFirstName} {child.childLastName}
              </div>
              <div style={S.childDob}>Date of birth: {child.dob}</div>
              <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 4, color: C.textMid }}>
                Who would you like to be {child.childFirstName}&apos;s guardian?
              </div>
              <div style={{ fontSize: 12, color: C.textLight, fontStyle: 'italic', marginBottom: 14 }}>
          ...
          <button type="button" style={S.backBtn} onClick={() => setChildrenConfirmed(false)}>
            ← Edit children
          </button>
        </div>
      )}
